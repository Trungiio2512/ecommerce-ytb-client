...
const ManagerCategory = (props) => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      ...
      setbrands(brands.data.map((brand) => ({ label: brand?.title, value: brand._id })));
      setLoading(false);
    };
    fetchApi();
  }, []);
  ...
  const hanldeCreate = async () => {
    if (!title || brands.length <= 0 || !image.url) {
      ...
      const rs = await apiCategories.create({
        title,
        image,
        brands: brandSelected.map((ele) => ele.value),
      });
      if (rs.sucess) {
  ...
  const hanldeValue = async (data) => {
    setBrandSelected(() => data.brands.map((ele) => ({ label: ele.title, value: ele?._id })));
    setTitle(data.title);
    if (data.image?.url) {
                      ...
                      <td className="px-6 py-4">
                        <div className="w-[100px] h-[100px]">
                          <figure className="w-full h-full">
                            <img src={cate?.image?.url ? cate?.image.url : cate?.image} alt="" />
                          </figure>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                          ...
                          <Tippy
                            ...
                            render={(attrs) => (
                              <div
                                ...
                                {...attrs}
                              >
                                {cate.brands.length > 0 ? (
                                  <ul>
                                    {cate?.brands.map((ele, index) => (
                                  ...
                                  <span className="text-sm text-third">Has not brands</span>
                                )}
                              </div>
                            )}
                          >
                            <span className="text-white text-sm cursor-pointer">
                              {cate?.brands.length} brand
                            </span>
                          </Tippy>
                        </div>
                      </td>
          ...
          <div className="sm:hidden block">
            <div className="grid-layout">
              <div className="row">
                {category.map((cate) => {
                  return (
                    <div className="col c-12 mb-4" key={cate?._id}>
                      <div className="w-full p-3 bg-white border border-gray-300 rounded-sm">
                        <h2 className="text-base text-third font-normal mb-2">{cate.title}</h2>
